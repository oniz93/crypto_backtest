...
def import_ticks():
    ...
    ticks_1m = pd.read_csv('data/BTCUSDT-1m.csv', names=columns)
    ticks_5m = pd.read_csv('data/BTCUSDT-5m.csv', names=columns)
...
def calcSR():
    ...
    if (not os.path.exists("data/support_resistance_5m.csv")):
        res = 0
        sup = 0
        inside_time = 0
        c = 0
        ...
        df.to_csv("data/support_resistance_5m.csv", header=True)
...
def calcAlpha():
    ...
    global N_TICKS_DELTA
    global N_TICKS_DELTA_NEG
    if (not os.path.exists("data/alpha_" + str(int(N_TICKS_DELTA)) + "_" + str(int(N_TICKS_DELTA)) + ".csv")):
        ...
        df.to_csv("data/alpha_" + str(int(N_TICKS_DELTA)) + "_" + str(int(N_TICKS_DELTA_NEG)) + ".csv", header=True)
...
def read_next_trade():
    ...
    if id_trade == 0:
        ...
        if (os.path.exists("data/BTCUSDT-trades-slim-2022-" + month_string.zfill(2) + ".csv")):
            columns = ['id', 'price', 'timestamp']
            trades = pd.read_csv("data/BTCUSDT-trades-slim-2022-" + month_string.zfill(2) + ".csv", names=columns,
                                 index_col='id').to_dict()
            trades['price'] = list(trades['price'].values())
            trades['timestamp'] = list(trades['timestamp'].values())
        else:
            columns = ['id', 'price', 'i1', 'i2', 'timestamp', 'i4', 'i5']
            trades = pd.read_csv("data/BTCUSDT-trades-2022-" + month_string.zfill(2) + ".zip", names=columns,
                                 index_col='id')
            ...
            trades.to_csv("data/BTCUSDT-trades-slim-2022-" + month_string.zfill(2) + ".csv", header=False)
            trades = trades.to_dict()
            trades['price'] = list(trades['price'].values())
            trades['timestamp'] = list(trades['timestamp'].values())
    try:
        last_price = trades['price'][id_trade]
        current_time = trades['timestamp'][id_trade]
...
# https://www.bitmex.com/api/v1/funding?symbol=XBTUSD&count=500&reverse=false&startTime=2022-01-01%2000%3A00%3A00
def getFundingRate():
    global funding_rates
    global funding_rate
    global old_min_fr
...
def placeOrderSell(exchange, qty):
    ...
    getOpenPositions(exchange, ["XBTUSD"])
...
def getLastPrice(exchange):
    ...
    while c < 10:
        try:
            trades = exchange.watch_trades(symbol='XBTUSD', since=(exchange.milliseconds() - (60 * 1000)))
...
def fetchPrice(exchange):
    ...
    max_unrealized_pnl = 0
    min_unrealized_pnl = 0
    getAvailableBalance(exchange, 'XBt')
...
def calculateProfit(config={}):
    ...
    while last_price >= 0:
        ...
        if last_price == -1:
            ...
            return write_orders()
        try:
            getOpenPositions(bitmex, ["XBTUSD"])
            if current_order['quantity'] == 0:
...
def fetchAlpha(exchange, config={}):
    ...
    if cur_min != old_min_alpha:
        alphavalues = calc_alpha['close_time'].values
        row = calc_alpha[ne.evaluate('(alphavalues <= current_time)')][-1:].values.tolist()
        average = row[0][1]
        average_neg = row[0][2]
...
def import_precalc():
    ...
    calc_sr = pd.read_csv("data/support_resistance_5m.csv")
    calc_alpha = pd.read_csv("data/alpha_" + str(int(N_TICKS_DELTA)) + "_" + str(int(N_TICKS_DELTA_NEG)) + ".csv")
...
PCT_NEW_LEVEL_MIN, PCT_NEW_LEVEL_MAX = 90, 110
TICK_DELTA_MIN, TICK_DELTA_MAX = 4, 50
TICK_DELTA_NEG_MIN, TICK_DELTA_NEG_MAX = 50, 200
PCT_VOL_DIST_MIN, PCT_VOL_DIST_MAX = 0.1, 15
PCT_VOL_DIST_NEG_MIN, PCT_VOL_DIST_NEG_MAX = 0.1, 15
...
if __name__ == '__main__':
    c = {'treshold_price': 3.421754469427168e-05, 'pct_new_level': 97.58915477792267, 'N_TICKS_DELTA': 33.0, 'N_TICKS_DELTA_NEG': 78.0,
         'PCT_VOLUME_DISTANCE': 4.2210845548725455, 'PCT_VOLUME_DISTANCE_NEGATIVE': 10.92536521990974, 'MULT_NEG_DELTA': 90.25662948520636,
         'N_REBUY': 8.0, 'DEFAULT_PCT_STOP_LOSS': -71.30850554508999, 'PCT_QTY_ORDER': 20.999054310885903, 'result': 583.5287}
    calculateProfit(c)
    # main()
